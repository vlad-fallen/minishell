...
/*   Updated: 2022/06/05 21:16:06 by mbutter          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_token_to_table(t_token **list_token, t_table_cmd **table)
{
	...
	tmp = *list_token;
	i = 0;
	while (tmp != NULL && (tmp->key == e_word || tmp->key == e_single_quote
			|| tmp->key == e_double_quote))
	{
		i++;
		tmp = tmp->next;
	}
	...
	if (tmp_array == NULL)
		return ;
	i = 0;
	while ((*list_token) != NULL && ((*list_token)->key == e_word
